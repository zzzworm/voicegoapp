    ...
    repeat {
                ...
                if countDelete + countInsert > 1 {
                    if countDelete != 0 && countInsert != 0 {
                        ...
                        if prefixLength != 0 {
                            ...
                            textInsert = String(textInsert.suffix(from: textInsert.index(textInsert.startIndex, offsetBy: prefixLength)))
                            textDelete = String(textDelete.suffix(from: textDelete.index(textDelete.startIndex, offsetBy: prefixLength)))
                        }
                        ...
                        if suffixLength != 0 {
                            ...
                            textInsert = String(textInsert.prefix(insertLength - suffixLength))
                            textDelete = String(textDelete.prefix(deleteLength - suffixLength))
                        }
                    }
        ...
        while pointer < diffs.count - 1 {
            if case .equal = diffs[pointer - 1], case .equal = diffs[pointer + 1] {
                ...
                let suffixStart = diffs[pointer].text.count - diffs[pointer - 1].text.count
                let suffix = String(diffs[pointer].text.suffix(from: diffs[pointer].text.index(diffs[pointer].text.startIndex, offsetBy: suffixStart)))
                ...
                if suffix == previousDiff.text {
                    ...
                    diffs[pointer] = diff.with(text:
                        previousDiff.text
                            + String(diff.text.prefix(diff.text.count - previousDiff.text.count)))
                    diffs[pointer + 1] =
                    ...
                    let prefix = String(diff.text.prefix(length))
                    if prefix == nextDiff.text {
                        ...
                        diffs[pointer] = diffs[pointer].with(text:
                            String(diffs[pointer].text.suffix(from: diffs[pointer].text.index(diffs[pointer].text.startIndex, offsetBy: nextDiff.text.count))) +
                                nextDiff.text)
                        diffs.remove(at: pointer + 1)
                        changes = true
                    }
    ...
    while pointer < diffs.count - 1 {
            ...
            case .equal(var equality2) = diffs[pointer + 1]
        {
            ...
            if commonOffset != 0 {
                ...
                equality1 = String(equality1.prefix(equality1.count - commonOffset))
